...
app = FastAPI(title="LinkedIn Read Gateway", version="1.0.3")
...
async def _call(tool: str, arguments: dict[str, Any]) -> Any:
    ...
    try:
        async with streamablehttp_client(MCP_URL, headers={"Host": MCP_HOST_HEADER}) as (read_stream, write_stream, _):
...
async def jobs(token: str, request: Request, keywords: str = Query(..., min_length=1, max_length=200), location: str | None = Query(default=None, max_length=120), max_pages: int = Query(default=2, ge=1, le=5), date_posted: str | None = Query(default=None, max_length=40)) -> Any:
    _authorize(token)
    _rate_limit(request)
...
async def posts(token: str, request: Request, keywords: str = Query(..., min_length=1, max_length=200), recency: str | None = Query(default="past-week", max_length=40), max_pages: int = Query(default=2, ge=1, le=5)) -> Any:
    _authorize(token)
    _rate_limit(request)
    args: dict[str, Any] = {"keywords": keywords, "max_pages": max_pages}
    if recency:
...
async def people(token: str, request: Request, keywords: str = Query(..., min_length=1, max_length=200), location: str | None = Query(default=None, max_length=120), max_pages: int = Query(default=2, ge=1, le=5)) -> Any:
    _authorize(token)
    _rate_limit(request)
    args: dict[str, Any] = {"keywords": keywords, "max_pages": max_pages}
    if location:
...
async def companies(token: str, request: Request, keywords: str = Query(..., min_length=1, max_length=200), max_pages: int = Query(default=2, ge=1, le=5)) -> Any:
    _authorize(token)
    _rate_limit(request)
...
async def company_employees(token: str, request: Request, name: str = Query(..., min_length=1, max_length=150), keywords: str | None = Query(default=None, max_length=150)) -> Any:
    _authorize(token)
    _rate_limit(request)
